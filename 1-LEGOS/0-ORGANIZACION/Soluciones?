...
DNS: COPIAR EL ARCHVO CONFIGURADO EN
OJO NOTE la "ed" en resolved.conf que NO ES resolv.conf archivo en /etc que se guarda por compatibilidad
resolvectl tiene su configuracion en /etc/systemd/resolved.conf y enlaza simbolicamente al archivo antiguo  para que puedan leerlo las aplicaciones antiguas que todavía lo utilizan
/etc/systemd/resolved.conf
en resumen:

para saber que DNS TENGO
resolvectl status
...
para reiniciar el servicio de resolución de nombres:
